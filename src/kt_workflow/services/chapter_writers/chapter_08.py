"""中游章节 Writer — 第八章 — 风险分析与应对

================================================================================
【你在改什么】
================================================================================
本文件对应 LangGraph 节点 `kt_chapter_08`，在流水线中的位置：

  … → kt_structure → **kt_chapter_08** → … → kt_aggregate → …
...
你要写的具体内容：5 个小节：四类风险 + 一张风险矩阵表（8.5）。
...
【本章模块清单（slug = id，勿改）】
================================================================================
#   - bp_ch8_8_1 | ref=8.1 | chart=否 | 技术风险
#   - bp_ch8_8_2 | ref=8.2 | chart=否 | 市场风险
#   - bp_ch8_8_3 | ref=8.3 | chart=否 | 政策风险
#   - bp_ch8_8_4 | ref=8.4 | chart=否 | 运营风险
#   - bp_ch8_8_5 | ref=8.5 | chart=是 | 风险矩阵表

字数/篇幅：8.1～8.4 各 ≤400；8.5 ≤300
话术模板：templates.md §八
注册表  ：config/bp_transform_outline_registry.json（chapter=8）
...
class Chapter08Writer(BaseChapterWriter):
    """chapter=8 的 Writer。组员只改本类，不要改 chapter_runner。"""
    ...
    def generate_module_chart(self, ctx: ChapterWriterContext):
        ...
        if module_id == "bp_ch8_8_5":
            return self._chart_bp_ch8_8_5(ctx)
        return self.scaffold_module_chart(ctx)

    def _chart_bp_ch8_8_5(self, ctx: ChapterWriterContext):
        """
        【图表占位】8.5 风险应对与风险矩阵
        slug=bp_ch8_8_5（与正文 bp_module_text 相同）
        建议 chart_type: table  writer_hint: 风险矩阵表
